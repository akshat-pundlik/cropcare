...
# 1. MODEL PATH: Ensure 'trained_plant_disease_model.keras' is in the same directory.
...
# 🚨 MUST EDIT: REPLACE THIS LIST WITH YOUR ACTUAL CLASS NAMES 🚨
# The order MUST match the order used when the model was trained!
# Example below assumes a 38-class dataset:
CLASS_NAMES = [
    'Apple__Black_rot',
    'Apple__Healthy',
    'Corn_(maize)__Common_rust',
    'Corn_(maize)__healthy',
    # ... List ALL 38 (or your specific count) categories here! ...
    'Tomato__Mosaic_virus',
    'Tomato__healthy'
]

# 🚨 MUST EDIT: Update this to the actual path of the image you want to test.
...
def predict_disease(image_path_to_predict):
    ...
    try:
        print("Loading model...")
        # load_model handles the new .keras format correctly
        model = load_model(MODEL_PATH) 
        print("Model loaded successfully.")
    except Exception as e:
        print(f"Error loading model: {e}")
        print(f"FATAL: Ensure '{MODEL_PATH}' is correct and in the same directory.")
        print("Also, confirm your TensorFlow version supports the Keras 3 .keras format.")
        return
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        # If an argument is provided, use it as the test image path
